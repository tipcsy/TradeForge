...
check("a P&L-ben tizedesVESSZO es elojel",
      tl.fmt("P&L", 1234.5) == "+1" + tl.NBSP + "234,50", tl.fmt("P&L", 1234.5))
check("az R elojeles, ket tizedes", tl.fmt("R", -1.5) == "-1,50", tl.fmt("R", -1.5))
check("az ido rovid alakban", tl.fmt("nyitás", rows[0]["nyitás"]) == "08-01 10:00",
      tl.fmt("nyitás", rows[0]["nyitás"]))
...
_t_desc = [r["nyitás"] for r in tl.sort_rows(rows, "nyitás", True)]
...
check("a Stop szuro NEM hozza a BE/trail zarast",
      [r["ok"] for r in tl.apply_filters(rows, reason="sl")] == ["sl"])
...
check("a CSV fejleces es teljes", _lines[0] == ";".join(tl.COLS)
